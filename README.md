...
3. Project Settings → API Keys → copy the **secret** key into `SUPABASE_SECRET_KEY`
   (previously documented but unused — account deletion now genuinely needs it,
   to remove the auth user itself via the admin API, which the regular
...
7. Run `npx prisma migrate dev` to create the app tables (`profiles`, `foods`,
